...
public class RetOps<T> {
	...
	/** 状态码为成功 */
	public static final Predicate<ApiResult<?>> CODE_SUCCESS = r -> ApiCode.SUCCESS.getCode() == r.getCode();
	...
	public boolean isSuccess() {
		return codeEquals(ApiCode.SUCCESS.getCode());
	}
	...
	public <Ex extends Exception> RetOps<T> assertSuccess(Function<? super ApiResult<T>, ? extends Ex> func) throws Ex {
		return assertCode(ApiCode.SUCCESS.getCode(), func);
	}
	...
	public <U> RetOps<U> map(Function<? super T, ? extends U> mapper) {
		ApiResult<U> result = ApiResult.result(ApiCode.getApiCode(original.getCode()), original.getMessage(), mapper.apply(original.getData()));
		return of(result);
	}
